...
| `CONTACT_FROM` | non | Expéditeur (défaut `POST <wearepostagency@gmail.com>`) |
...
2. **Vérifier le domaine `postwtf.vercel.app`** (*Domains* → enregistrements DNS
   SPF/DKIM). Sans domaine vérifié, l'envoi depuis `wearepostagency@gmail.com` est
